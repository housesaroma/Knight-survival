...
namespace Knight_survival.GameScene
{
    internal partial class GameScene
    {
        ...
        private Monster ChooseMonsterToSpawn()
        {
            ...
            int mushroomWeight = Math.Max(0, 100 - (int)(timeFactor * 100)); // Вес для грибов уменьшается со временем
            int eyeWeight = (int)Math.Max(0, Math.Min(50, timeFactor * 50)); // Вес для глаз начинает увеличиваться и достигает максимума в 50
            int goblinWeight = (int)Math.Max(0, Math.Min(50, (timeFactor - 0.25) * 50)); // Вес для гоблинов начинает увеличиваться после 15 сек
            int skeletonWeight = (int)Math.Max(0, Math.Min(50, (timeFactor - 0.5) * 50)); // Вес для скелетов начинает увеличиваться после 30 сек
